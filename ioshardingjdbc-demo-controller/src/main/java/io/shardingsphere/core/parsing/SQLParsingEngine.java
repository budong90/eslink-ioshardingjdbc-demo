...
public final class SQLParsingEngine {
    ...
    public SQLStatement parse(final boolean useCache) {
        ...
        if (result instanceof SelectStatement) {
            SelectStatement selectResult = (SelectStatement) result;
            Iterator<OrCondition> it = selectResult.getSubQueryConditions().iterator();
            while (it.hasNext()) {
                OrCondition cond = it.next();
                if (cond.getAndConditions().isEmpty()) {
                    it.remove();
                }
            }
        }
